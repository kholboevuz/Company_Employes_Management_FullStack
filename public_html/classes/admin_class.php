...
/* -------------------------set_database_connection_using_PDO---------------------- */
	...
	{ 
        $host_name='localhost';
		$user_name='cc56635_text';
		$password='EDKNzJa7';
		$db_name='cc56635_text';
...
/* ---------------------- Admin Login Check ----------------------------------- */
    ...
    public function change_password_for_employee($data){
		...
		if($password == $re_password){
			...
			$message = 'Kechirasiz!! Parol mos kelmaydi';
            return $message;
		}
	...
	public function add_new_user($data){
		...
		try{
			...
			if($total_email != 0 && $total_username != 0){
				$message = "Lavozim va parol allaqachon olingan";
            	return $message;

			}elseif($total_username != 0){
				$message = "Foydalanuvchi nomi allaqachon olingan";
            	return $message;

			}elseif($total_email != 0){
				$message = "Lavozim allaqachon olingan";
            	return $message;
...
/* -------------admin_password_change------------*/
	...
	public function admin_password_change($data, $id){
		...
		try{
			...
			$all_error = '';
			if($total_row == 0){
				$all_error = "Eski parol yaroqsiz";
			}
			

			if($admin_new_password != $admin_cnew_password ){
				$all_error .= '<br>'."Yangi va Yangi parolni tasdiqlash mos kelmaydi";
			}

			$password_length = strlen($admin_raw_password);

			if($password_length < 6){
				$all_error .= '<br>'."Parol uzunligi 6 ta belgidan kam bulmasligi kerak";
			}
	...
	public function add_new_task($data){
		...
		try{
			...
			$_SESSION['Task_msg'] = 'Vazifa muvaffaqiyatli qoshildi';
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				...
				$_SESSION['Task_msg'] = 'Vazifa muvaffaqiyatli yangilandi';
